...
public class GeminiService {
  ...
  public JsonNode getParsedReceipt(String rawText, byte[] imageBytes, String mimeType) {
    String model = "models/gemini-3.5-flash";
    String url =
    ...
    String prompt =
        "너는 한국 영수증 데이터 추출 전문가야.\n"
            ...
            + "OCR 텍스트와 이미지 내용이 다를 경우, 반드시 이미지를 우선하여 판단해줘.\n"
            ...
            + "   - 영수증에 인쇄된 원문 표기를 그대로 사용. 번역/변환 금지.\n"
            + "     예) COSTCO WHOLESALE → COSTCO WHOLESALE (한국어로 바꾸지 말 것)\n"
            + "     예) emart → emart (이마트로 바꾸지 말 것)\n"
            + "   - 주문번호/대기번호/테이블번호는 storeName으로 사용 금지.\n"
            + "     예) '[식사] 240번', '대기번호 357', '주문번호 256' 등은 상호명이 아님.\n"
            + "     '제품받는곳', '주문번호가 표시되면' 같은 안내 문구도 무시.\n"
            + "     영수증 상단에 큰 글씨로 표시된 숫자/번호는 대부분 주문번호이므로 상호명이 아님.\n"
            + "     상호명은 로고 또는 사업자번호 근처에 표시된 가게 이름을 사용.\n"
            + "   - 푸드코트 영수증의 경우 Food Street, YAWOORI 등 공통 상호명이 아닌 실제 입점 가게명 사용.\n"
            + "     사업자번호가 여러 개인 경우 가장 마지막(하단) 사업자번호 바로 위에 있는 상호명이 실제 입점 가게명임.\n"
            + "   - 대표자명은 storeName으로 사용하지 말 것.\n"
            + "     사업자번호(XXX-XX-XXXXX) 옆이나 같은 줄의 이름은 대표자명임.\n"
            + "     '대표자:', '대표:' 옆 이름도 대표자명임.\n"
            + "     TEL/전화번호 옆이나 같은 줄의 한국 이름(2~4글자)은 대표자명임.\n"
            + "     CASHIER 옆 이름은 직원명이므로 storeName 사용 금지.\n"
            + "     상호명은 영수증 최상단에 가장 크게 표시된 가게 이름을 우선 사용.\n"
            + "   - 동일 브랜드명이 한국어+영문으로 중복 표기된 경우 하나만 사용. 지점명은 반드시 유지.\n"
            + "     예) emart 이마트 김포한강점 → emart 김포한강점 (브랜드 중복만 제거, 지점명 유지)\n"
            + "     예) MUJI 無印良品 신세계 스타필드 고양점 → MUJI 신세계 스타필드 고양점\n"
            + "     예) TRADERS 트레이더스 홀세일 클럽 고양점 → 트레이더스 홀세일 클럽 고양점\n"
            + "   - 한국어로 표기된 상호명은 한국어 그대로. 영문 번역/변환 금지.\n"
            + "     예) 유니클로 → 유니클로 ✅ (UNIQLO로 바꾸지 말 것)\n"
            + "     예) 교보문고 → 교보문고 ✅ (Kyobobook으로 바꾸지 말 것)\n"
            + "   - 상호명에 언더바(_) 포함 금지.\n"
            + "     예) 하이파킹_명동중부점 → 하이파킹 명동중부점\n"
            + "   - 주차장·주유소는 운영사명이 아닌 실제 장소명 우선.\n"
            + "     예) 파킹클라우드(주) → 주차장 실제 명칭 사용\n"
            + "     예) 영수증 상단에 장소명(OO주차장, OO공영주차장)이 있으면 그것을 사용\n"
            + "   - 글자가 흐리거나 비슷하게 생긴 경우 이미지를 자세히 보고 정확한 글자 판단.\n"
            + "     예) 촌/존, 솔/솥, 차/자, 툰/뿐 등 유사 글자 주의.\n"
            ...
            + "   - 시간 없으면 '00:00:00' 사용\n"
            + "   - 연도는 현재 기준 최근 5년 범위여야 함.\n"
            + "     OCR 텍스트의 연도가 비현실적이면(예: 2006년, 2019년 등 과거) OCR을 무시하고 이미지에서 직접 날짜를 다시 읽을 것.\n"
            + "   - 시간은 0~23시 범위. 범위 벗어나면 이미지 재확인.\n"
            + "   - 결제일시는 카드 승인 시각 또는 영수증 발행 시각 우선.\n"
            + "     영화관 예매 시각, 입장 시각은 사용 금지.\n"
